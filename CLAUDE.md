...
openconv is still developed on GitHub (`git@github.com:promptctl/openconv.git`), and pull requests and code review stay there. The repo also carries a second remote pointing at its Gitea repo on `gitea.sanctuary.gdn`, and **pushing a commit to that remote is what triggers CI.** No mirror, no polling, no schedule — a run exists because someone deliberately pushed the ref they wanted built. `.gitea/workflows/ci-builder.yaml` runs on any push (no branch filter) and on manual dispatch, and today it proves the builder rather than building: checkout matches the triggering commit, runner is x86_64 with a BuildKit-capable Docker daemon, registry reads and writes. No image is built, tagged or pushed by it. That step — `docker build`, tag `YYYY.MM.DD.N`, push — is `openconv-deploy-690.3`, split off deliberately so the builder was proven before a long cold build was attempted.
